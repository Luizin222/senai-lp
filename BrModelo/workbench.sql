...
CREATE TABLE `jogador` (
 ...
 CREATE TABLE `estadio` (
  ...
  CREATE TABLE `partida` (
   ...
   KEY `FK_TimeMadante` (`id_mandante`),
   ...
   CONSTRAINT `FK_TimeMadante` FOREIGN KEY (`id_mandante`) REFERENCES `time` (`id_time`),
 ...
 CREATE TABLE `evento` (
   ...
   KEY `PK_EventoPartida` (`id_partida`),
   KEY `PK_EventoJogador` (`id_jogador`),
   CONSTRAINT `PK_EventoJogador` FOREIGN KEY (`id_jogador`) REFERENCES `jogador` (`id_jogador`),
   CONSTRAINT `PK_EventoPartida` FOREIGN KEY (`id_partida`) REFERENCES `partida` (`id_partida`)
 ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
 
 -- apagando regra de chave estrangeira
  ...
  alter table partida drop constraint FK_TimeMandante;
   alter table partida drop constraint FK_TimeVisitante;
    ...
    alter table jogador add constraint FK_JogadorTime foreign key(id_time) references time(id_time);
     alter table partida add constraint FK_TimeMadante foreign key(id_mandante) references time(id_time);
     alter table partida add constraint FK_TimeMadante foreign key(id_visitante) references time(id_time);
desc time;
desc partida;
